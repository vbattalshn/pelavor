...
export default function CreateList() {
  ...
  useEffect(() => {
    ...
    apiClient
      .post("/get-list-data", requestData)
      .then((response) => {
        setListData(JSON.parse(response.data.data.words));
        setTrash(JSON.parse(response.data.data.trash))
    ...
    <Layout>
      ...
      <main className="flex gap-2 flex-col lg:flex-row">
        <div className="w-full p-2 flex flex-col gap-2">
          ...
          <AddNewWord />
          <ListSettings listData={listData} trashData={trash} isUpdate={true} title={data.title} description={data.description} ListImage={data.image} url={slug}/>
        </div>
        <div className="w-full p-2 flex flex-col gap-2">
          ...
          <div className="bg-neutral-200/50 border border-neutral-200 rounded-lg flex flex-col transition gap-2 px-3 py-2 text-neutral-800 font-semibold">
            <span>
              Eklenen kelime sayısı: {listData.length}
            </span>
            <span>
              Çöp kutusundaki kelime sayısı: {trash.length}
            </span>

          </div>
          {!showTheTrash ? (
            <ListData data={listData} deleteWord={deleteWordinList} />
          ) : (
            <TrashData
              data={trash}
...
function ListData({ data, deleteWord }) {
  return (
    <div className="bg-neutral-200/50 border border-neutral-200 rounded-lg flex flex-col transition ">
      {data.length > 0 ? (
        data.map((item, index) => (
          <div key={index} className="border-b border-neutral-300 p-2 last:border-none flex justify-between items-center font-medium">
            {item}
            <button
...
function TrashData({ data, deleteWord, reloadWord }) {
  return (
    <div className="bg-neutral-200/50 border border-neutral-200 rounded-lg flex flex-col transition ">
      {data.length > 0 ? (
        data.map((item, index) => (
